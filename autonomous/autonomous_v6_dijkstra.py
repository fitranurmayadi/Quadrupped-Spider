...
from src.path_planning import GridMap, dijkstra_search, pure_pursuit
...
def main():
    print("==================================================")
    print("   V6: GLOBAL PATH PLANNING (DIJKSTRA) ")
    print("==================================================")
    ...
    print("  - Controller     : Dijkstra Path Planning + Pure Pursuit")
    ...
    for i in range(num_obstacles):
        ...
        while True:
            ...
            if np.linalg.norm(pos - start_point) > 2.0 and np.linalg.norm(pos - target_point) > 2.0:
                too_close = False
                for obs_pos, obs_w, obs_l in obstacles:
    ...
    # 4. Global Path Planning (Dijkstra)
    print("[INFO] Generating Occupancy Grid and running Dijkstra Search...")
    grid_map = GridMap(resolution=0.1, safe_margin=0.15)
    for pos, w, l in obstacles:
        grid_map.add_box_obstacle(pos[0], pos[1], w, l)
        
    global_path = dijkstra_search(grid_map, start_point, target_point)
    if not global_path:
        print("[ERROR] No path found by Dijkstra!")
        sys.exit(1)
        
    print(f"[INFO] Dijkstra Path found with {len(global_path)} waypoints.")
    
    # Draw global path
    for i in range(len(global_path) - 1):
        p.addUserDebugLine(
            [global_path[i][0], global_path[i][1], 0.05], 
            [global_path[i+1][0], global_path[i+1][1], 0.05], 
        ...
        while p.isConnected(physicsClientId=client):
            ...
            if escape_timer > 0.0:
                ...
                if dist_front < 0.60 or dist_left < 0.60 or dist_right < 0.60:
                    ...
                    # ZONE 3: Clear Zone (>= 60cm) -> Pure Pursuit Path Following
                    target_pt, heading_error, is_last = pure_pursuit(pos[:2], robot_heading_angle, global_path, lookahead_dist=0.45)
                    
                    if not headless and show_debug_lines:
                        p.addUserDebugLine([pos[0], pos[1], 0.1], [target_pt[0], target_pt[1], 0.1], [1, 0, 1], 2, lifeTime=0.1, physicsClientId=client)
                    
                    if np.abs(heading_error) > 1.2:
                        yaw_rate = np.clip(3.5 * heading_error, -1.5, 1.5)
                        dx, dy = 0.0, 0.0
                    else:
                        yaw_rate = np.clip(3.0 * heading_error, -1.5, 1.5)
                        forward_speed = 1.0 * max(0.3, np.cos(heading_error))
                        # Slow down near the end
                        dist_to_final = np.linalg.norm(np.array(pos[:2]) - target_point)
                        if dist_to_final < 0.6:
                            forward_speed *= (dist_to_final / 0.6)
                        dy = -forward_speed
